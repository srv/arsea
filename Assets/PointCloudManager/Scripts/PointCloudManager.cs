...
public class PointCloudManager : MonoBehaviour {
	...
	IEnumerator loadOFF(string dPath){
		...
		for (int i = 0; i< numPoints; i++){
			...
			if (i % Mathf.FloorToInt(numPoints / 20) == 0) {
				Debug.Log (i.ToString() + " out of " + numPoints.ToString() + " loaded");
				yield return null;
			}
		...
		for (int i = 0; i < numPointGroups-1; i ++) {
			...
			if (i%10 == 0) {
				Debug.Log (i.ToString() + " out of " + numPointGroups.ToString() + " PointGroups loaded");
				yield return null;
			}
		}
	...
	void OnGUI(){
		if (!loaded){
			Progress pbar = GameObject.FindGameObjectWithTag("ProgressBar").GetComponent<Progress>();
